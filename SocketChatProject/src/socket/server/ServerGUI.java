...
public class ServerGUI extends JFrame {
	...
	ImageIcon img = new ImageIcon("./image/socket_logo.png");
	...
	public ServerGUI() {
		...
		ServerPanel.portInput.setText(String.valueOf(numPort));
		ServerPanel.portInput.setEditable(false);
		ServerPanel.openRoomBtn.setEnabled(false);
...
class ServerPanel extends JPanel {
	...
	public static JButton openRoomBtn;
	...
	public ServerPanel() {
		...
		portInput = new JTextField(null, JLabel.LEFT);
		...
		// 내 정보 파트
		JLabel myInfoTitle = new JLabel("채팅창", JLabel.LEFT);
		...
		randomNickname.setBackground(Color.DARK_GRAY);
		...
		openRoomBtn = new JButton("채팅방 열기");
		...
		colseRoomBtn.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				int result = JOptionPane.showConfirmDialog(null, "정말 채팅방을 닫으시겠습니까?", "채팅방 닫기", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
				if(result == 0) {
					System.exit(0);
				}
			}
